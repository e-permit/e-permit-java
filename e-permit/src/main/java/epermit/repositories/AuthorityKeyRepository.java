...
public interface AuthorityKeyRepository extends JpaRepository<AuthorityKey, Integer> {
    ...
    @Query("SELECT ak FROM AuthorityKey ak WHERE ak.authority.code = :issuer and ak.keyId = :key_id")
    Optional<AuthorityKey> findOneByIssuerAndKeyId(@Param("issuer") String authorityCode,
            @Param("key_id") String keyId);
}
