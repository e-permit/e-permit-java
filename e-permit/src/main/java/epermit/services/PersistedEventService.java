...
public class PersistedEventService {
    ...
    @SneakyThrows
    public void handleLedgerEventReceived(LedgerEventReceived event) {
        log.info("Event handle started. Claims is {}", event.getClaims());
        LedgerEventHandleResult r = handleOne(event.getClaims());
        if (!r.isOk() && r.getErrorCode().equals("NOTEXIST_PREVIOUSEVENT")) {
            LedgerEventBase eBase = GsonUtil.fromMap(event.getClaims(), LedgerEventBase.class);
