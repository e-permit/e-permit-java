...
public class CreatePermitCommandHandler
        ...
        public CreatePermitCommandHandler(AppEventPublisher eventPublisher,
                        PermitService permitService, IssuedPermitRepository repository,
                        PermitProperties properties, PermitCreatedEventFactory factory,
        ...
        public CommandResult handle(CreatePermitCommand cmd) {
                Integer serialNumber = permitService.generateSerialNumber(cmd.getIssuedFor(),
                                cmd.getPermitYear(), cmd.getPermitType());
                if (serialNumber == null) {
                        return CommandResult.fail("NOT_SUFFICIENT_PERMITID",
                                        "Permit id is not sufficient");
                }
                IssuedPermit permit = Utils.convertCommandToPermit(cmd, properties.getIssuerCode(),
                                serialNumber);
        ...
        public static class Utils {
                public static IssuedPermit convertCommandToPermit(CreatePermitCommand cmd,
                                String issuer, Integer serialNumber) {
                        ...
                        permit.setExpireAt(OffsetDateTime.parse(expireDate, dtf).plusMonths(1)
                                        .format(dtf));
